...
import comboApi from '../services/comboApi.js'; // Import the new combo API
...
export const DataProvider = ({ children }) => {
      ...
      const fetchData = async () => {
        try {
          const [fetchedBranches, fetchedCategories, fetchedServices, fetchedEmployees, fetchedCombos] = await Promise.all([
            ...
            comboApi.fetchCombos() // Fetch combos from API
          ]);
          ...
          // Transform API combo data to match frontend format
          const transformedCombos = fetchedCombos.map(combo => ({
            id: combo.id,
            name: combo.name,
            description: combo.description,
            price: combo.totalPrice,
            finalPrice: combo.discountPrice,
            totalOriginalPrice: combo.totalPrice,
            savingsPercent: combo.discountPercentage,
            discountPercent: combo.discountPercentage,
            validityDays: combo.validityDays,
            status: combo.status,
            branchIds: [combo.branchId], // Convert single branchId to array
            branchId: combo.branchId,
            productIds: combo.services?.map(s => s.serviceId) || [],
            products: combo.services?.map(s => ({
              id: s.service.id,
              name: s.service.name,
              finalPrice: s.service.price,
              quantity: s.quantity
            })) || [],
            stock: Math.min(...(combo.services?.map(s => 50) || [50])), // Default stock
  ...
  // Combo operations (now API-driven)
  const addCombo = async (comboData) => {
    try {
      // Calculate derived values
      const selectedServices = services.filter(s => comboData.productIds.includes(s.id));
      const totalOriginalPrice = selectedServices.reduce((sum, s) => sum + s.price, 0);
      const finalPrice = comboData.offerPrice || comboData.price;
      const savingsPercent = totalOriginalPrice > 0 
        ? Math.round(((totalOriginalPrice - finalPrice) / totalOriginalPrice) * 100 * 100) / 100 
        : 0;
      ...
      const dataToSend = {
        ...
        finalPrice,
        savingsPercent,
        branchId: user.branchId || comboData.branchIds[0] // Use user's branch or first selected branch
      };

      const newCombo = await comboApi.addCombo(dataToSend);
      ...
      const transformedCombo = {
        ...
        totalOriginalPrice: newCombo.totalPrice,
        savingsPercent: newCombo.discountPercentage,
        discountPercent: newCombo.discountPercentage,
        validityDays: newCombo.validityDays,
        status: newCombo.status,
        branchIds: [newCombo.branchId],
        branchId: newCombo.branchId,
        productIds: selectedServices.map(s => s.id),
        products: selectedServices,
        stock: Math.min(...selectedServices.map(s => 50)), // Default stock
  ...
  const updateCombo = async (id, comboData) => {
    try {
      ...
      const updatedCombo = await comboApi.updateCombo(id, dataToSend);
      ...
      const transformedCombo = {
        ...
        totalOriginalPrice: updatedCombo.totalPrice,
        savingsPercent: updatedCombo.discountPercentage,
        discountPercent: updatedCombo.discountPercentage,
