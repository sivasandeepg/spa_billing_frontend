...
const POS = () => {
  const { user } = useAuth();
  const { combos, addTransaction, addCustomer, branches, transactions } = useData();
  ...
  //     const isInBranch = service.branchIds && service.branchIds.includes(user?.branchId);
  //     const matchesSearch = service.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
  //                           service.description.toLowerCase().includes(searchTerm.toLowerCase());
      
  //     return isInBranch && matchesSearch;
  //   }).map(service => ({ ...service, isService: true, finalPrice: service.price }));
  ...
  //   return [...availableServices, ...availableCombos];
  // }, [services, combos, user?.branchId, searchTerm]);
 
      
const availableItems = useMemo(() => {
  const availableServices = services.filter(service => {
    // Removed the isInBranch condition to show all services
    const matchesSearch = service.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      service.description.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const availableCombos = combos.filter(combo => {
    // Removed the isInBranch condition to show all combos
    const hasStock = combo.stock > 0;
    const matchesSearch = combo.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      combo.description.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  return [...availableServices, ...availableCombos];
}, [services, combos, searchTerm]); // Removed user?.branchId from the dependency array  
  ...
  const addToCart = (itemToAdd) => {
    ...
    if (existingItem) {
      if (itemToAdd.isCombo && existingItem.quantity >= itemToAdd.stock) {
        alert('Insufficient stock!');
        return;
      }
      setCart(cart.map(item => item.id === itemToAdd.id ? { ...item, quantity: item.quantity + 1 } : item));
    } else {
  ...
  const updateQuantity = (itemId, newQuantity) => {
    if (newQuantity === 0) {
      ...
      if (itemToUpdate.isCombo && newQuantity > itemToUpdate.stock) {
        alert('Insufficient stock!');
        return;
      }
  ...
  const handleCustomerSubmit = (e) => { e.preventDefault(); const newCustomer = addCustomer(customerForm); setCustomer(newCustomer); setIsCustomerModalOpen(false); setCustomerForm({ name: '', phone: '', email: '' }); };
