...
const ComboManagement = () => {
  ...
  const availableServices = useMemo(() => {
    if (!services || authLoading) return [];
  ...
  const filteredCombos = useMemo(() => {
    ...
    return filtered;
  }, [combos, searchTerm, statusFilter, user]);

  const handleSubmit = async (e) => {
    e.preventDefault();
    setIsLoading(true);
    
    if (!Array.isArray(formData.serviceIds) || formData.serviceIds.length < 2) {
    ...
    try {
      ...
      const comboData = {
        ...
        totalPrice: totalPrice, // <-- CORRECTED: Include totalPrice
        ...
        serviceIds: formData.serviceIds, // <-- CORRECTED: Send the array of IDs
      };
      ...
      if (result) {
        ...
        alert('Failed to save combo. Please check the data and try again.');
      }
    } catch (error) {
      console.error('Error saving combo:', error);
      const errorMessage = error.message || 'Failed to save combo. Please try again.';
      alert(errorMessage);
    } finally {
      setIsLoading(false);
    }
  ...
  const calculatePreview = () => {
    if (!formData.serviceIds || !Array.isArray(formData.serviceIds)) {
      return { selectedServices: [], totalPrice: 0, discountPrice: 0, savings: 0, savingsPercent: 0 };
    }
              ...
              <div className="mb-4">
                ...
                <div className="space-y-1 mb-3">
                  ...
                  {combo.services?.map((comboService, index) => (
                    <div key={comboService.service?.id || index} className="flex justify-between text-sm">
                      <span className="text-gray-700">
                        {comboService.service?.name || 'Unknown Service'}
                        {comboService.quantity > 1 && ` (x${comboService.quantity})`}
                      </span>
                      <span className="text-gray-500">₹{comboService.service?.price?.toFixed(2) || '0.00'}</span>
                    </div>
                  ))}
                </div>
                  ...
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <div className="text-right">
                      <span className="text-xl font-bold text-green-600">
                        ₹{combo.discountPrice?.toFixed(2) || '0.00'}
                      </span>
            ...
            <div className="bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-6">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="border rounded-lg p-3 max-h-64 overflow-y-auto space-y-2">
                        {availableServices.map(service => (
                          <label key={service.id} className="flex items-center p-2 hover:bg-gray-50 rounded">
                            <input
                              type="checkbox"
                              checked={Array.isArray(formData.serviceIds) && formData.serviceIds.includes(service.id)}
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        Selected: {Array.isArray(formData.serviceIds) ? formData.serviceIds.length : 0} services
                      </p>
                    </div>
                ...
                <div className="flex justify-end space-x-3 pt-4">
                  ...
                  <button
                    type="submit"
                    disabled={!Array.isArray(formData.serviceIds) || formData.serviceIds.length < 2 || isLoading}
